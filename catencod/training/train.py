...
"Totally didn't get this from https://github.com/microsoft/EdgeML/blob/master/examples/pytorch/DROCC/main_cifar.py"
"I did modify it a bit for custom datasets"
...
def main():
    ...
    test_path = "Mydataset/val"

    trainset = ImageFolder(root=train_path, transform=transform, target_transform=target_transform, is_valid_file=check_Image)
    testset = ImageFolder(root=test_path, transform=transform, target_transform=target_transform, is_valid_file=check_Image)

    train_loader, test_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True), DataLoader(testset, batch_size=args.batch_size, shuffle=False)
    print("Data Loaded")
